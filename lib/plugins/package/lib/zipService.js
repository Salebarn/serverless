...
function excludeNodeDevDependencies(servicePath) {
  ...
  try {
    ...
    _.forEach(relevantFilePaths, (relevantFilePath) => {
      ...
      const outputFile = path.join(os.tmpdir(), 'deps.tmp');
      childProcess.execSync(`npm ls --prod=true --parseable=true --silent > ${outputFile}`);

      const prodDependencies = fs.readFileSync(outputFile, 'utf8');
